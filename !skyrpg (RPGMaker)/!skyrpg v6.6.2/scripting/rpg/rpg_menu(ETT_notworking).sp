stock BuildMenuTitle(client, Handle:menu, bot = 0, type = 0, bool:bIsPanel = false) {	// 0 is legacy type that appeared on all menus. 0 - Main Menu | 1 - Upgrades | 2 - Points
	...
	if (bot == 0) {
		...
		if (CheckRPGMode > 0) {
			...
			Format(TheClass, sizeof(TheClass), "%T", "none", client);
			...
			if (CheckRPGMode != 0) Format(text, sizeof(text), "%T", "RPG Header", client, PlayerLevelText, TotalPoints, PlayerLevel[client] - 1, (PlayerLevel[client] - 1) - TotalPoints);
			...
			Format(stext, sizeof(stext), "%s\n \nRating: %s\nDifficulty: %s", stext, AddCommasToString(Rating[client]), AddCommasToString(TruRating(client)));
...
stock LoadProfileEx(client, String:key[]) {
	...
	if (IsSurvivorBot(LoadTarget[client]) || IsSurvivorBot(client) || LoadTarget[client] == -1 || IsLegitimateClient(LoadTarget[client]) && GetClientTeam(LoadTarget[client]) == TEAM_SURVIVOR) {
		...
		if (IsSurvivorBot(targetClient) || !bIsInCombat[targetClient] && !IsLoadingClassData[targetClient]) {

			if (IsLegitimateClass(targetClient)) {

				Format(ProfileLoadQueue[targetClient], sizeof(ProfileLoadQueue[]), "%s", key);
				SaveClassData(targetClient);
			}
			else LoadProfileEx_Confirm(targetClient, key);
		}
	}
...
public QueryResults_LoadTalentTreesEx(Handle:owner, Handle:hndl, const String:error[], any:client) {
	...
	if (hndl != INVALID_HANDLE) {
		...
		while (SQL_FetchRow(hndl)) {
			...
			if (LoadPos[client] < GetArraySize(a_Database_Talents)) {
				...
				if (LoadPos[client] < GetArraySize(a_Database_Talents)) {
					...
					Format(tquery, sizeof(tquery), "SELECT `steam_id`, `aclass`, `primarywep`, `secondwep` FROM `%s` WHERE (`steam_id` = '%s');", TheDBPrefix, key);
				...
				//if (!bIsInCombat[client] || IsFakeClient(client)) {

				Format(ActiveClass[client], sizeof(ActiveClass[]), "%s", text);
		...
		if (IsSurvivorBot(client) && (PlayerLevel[client] < iPlayerStartingLevel || !IsLegitimateClass(client))) {
...
stock GetTeamComposition(client) {
	...
	for (new i = 1; i <= MaxClients; i++) {
		...
		if (!IsLegitimateClass(i) || iIsClassesEnabled != 1) Format(Classname, sizeof(Classname), "Vanilla");
		else Format(Classname, sizeof(Classname), "%T", ActiveClass[i], client);

		GetClientName(i, text, sizeof(text));
		Format(text, sizeof(text), "[Rating: %s] (TLv.%s)(%s Lv.%s) %s", AddCommasToString(Rating[i]), AddCommasToString(PlayerLevel[i]), Classname, AddCommasToString(CartelLevel(i)), text);
		AddMenuItem(menu, text, text);
	}
	SetMenuExitBackButton(menu, true);
...
public ReadProfilesMenuHandle(Handle:menu, MenuAction:action, client, slot) {
	...
	if (action == MenuAction_Select) {
		...
		new target = client;
		if (LoadTarget[client] != -1 && LoadTarget[client] != client && (IsSurvivorBot(LoadTarget[client]) || !bIsInCombat[LoadTarget[client]])) target = LoadTarget[client]; 

		if (StringToInt(text) < GetArraySize(PlayerProfiles[client]) &&
			(!bIsInCombat[client] || target != client) &&
			!IsLoadingClassData[target]) {
...
stock Float:CheckActiveAbility(client, thevalue, eventtype = 0, bool:IsPassive = false, bool:IsDrawEffect = false, bool:IsInstantDraw = false) {
	...
	decl String:text[64], String:Effects[64], String:none[64], String:sTalentName[64], String:sDrawEffect[PLATFORM_MAX_PATH];	// free guesses on what this one is for.
...
public BuildMenuHandle(Handle:menu, MenuAction:action, client, slot) {
	...
	if (action == MenuAction_Select) {
		...
		if (!StrEqual(t_MenuName, "-1", false) && iIsIgnoreHeader <= 0) {
...
stock BuildSubMenu(client, String:MenuName[], String:ConfigName[], String:ReturnMenu[] = "none") {
	...
	decl String:sTalentRequirements[512];
	decl String:sTalentTreeRequirements[64];
	...
	for (new i = 0; i < size; i++) {
		...
		if (isSubMenu == 1 || isSubMenu == 2) {
			...
			if (ClassRole == 1 && iIsClassesEnabled != 1) continue;
			...
			if (ClassRole != 1 && isSubMenu != 1) {

				FormatKeyValue(sClassAllowed, sizeof(sClassAllowed), MenuKeys[client], MenuValues[client], "classes allowed?");
			...
			FormatKeyValue(sTalentRequirements, sizeof(sTalentRequirements), MenuKeys[client], MenuValues[client], "talents required?");
			FormatKeyValue(sTalentTreeRequirements, sizeof(sTalentTreeRequirements), MenuKeys[client], MenuValues[client], "talent tree category?");
			if (!StrEqual(sTalentRequirements, "-1")) {

				Format(sTalentRequirements, sizeof(sTalentRequirements), "%s", IsTalentUnlocked(client, sTalentRequirements, sTalentTreeRequirements, GetKeyValueFloat(MenuKeys[client], MenuValues[client], "talents required in tree?"), GetKeyValueInt(MenuKeys[client], MenuValues[client], "minimum tree level?")));
			}
			...
			if (ClassRole == 1 || AbilityTalent == 1) {
				...
				if (!b_IsDirectorTalents[client]) {
					...
					//if (!TalentRequirementsMet(client, sTalentsRequired)) {
					if (!StrEqual(sTalentRequirements, "-1")) {

						bIsNotEligible = true;
						if (PlayerTalentPoints > 0) {

							FreeUpgrades[client] += PlayerTalentPoints;
						...
						if (isSpecialAmmo == 1 && bEquipSpells[client] && PlayerTalentPoints < 1) continue;
			...
			/*if (bIsNotEligible) {
				...
				if (iShowLockedTalents == 0 || AbilityTalent == 1) continue;	// abilities that players don't have the talent requirements for do not show.
				Format(text, sizeof(text), "%T", "Talent Requirements Not Met", client, TalentName_Temp);
				if (PlayerLevel[client] < TalentLevelRequired) Format(text, sizeof(text), "%s (Lv.%d+)", text, TalentLevelRequired);

				TalentRequirementsMet(client, sTalentsRequired, TalentName_Temp, sizeof(TalentName_Temp));
				Format(text, sizeof(text), "%s\n%s", text, TalentName_Temp);
			}*/
			if (ClassRole != 1 && AbilityTalent != 1) {
...
stock String:IsTalentUnlocked(client, String:sTalentRequirements[], String:sTalentCategoryRequired[], Float:fTalentTreeLevel, iMinLevel) {

	decl String:sTalentInformation[64];
	Format(sTalentInformation, sizeof(sTalentInformation), "-1");
	new iTalentLevelRequired = RoundToNearest(PlayerLevel[client] * fTalentTreeLevel);
	if (iTalentLevelRequired < iMinLevel) iTalentLevelRequired = iMinLevel;
	new iTalentsInCategory = GetUpgradesByCategory(client, sTalentCategoryRequired);

	new PlayerTalentPoints = 0;
	new iTalentsRequired = GetDelimiterCount(sTalentRequirements, ":") + 1;
	decl String:tTalentName[iTalentsRequired][64];
	ExplodeString(sTalentRequirements, ":", tTalentName, iTalentsRequired, 64);
	//decl String:tStrengthRequired[2][64];
	decl String:tTranslation[64];
	decl String:tTalentNameInfo[2][64];

	if (iTalentsInCategory < iTalentLevelRequired) {

		Format(tTranslation, sizeof(tTranslation), "%T", sTalentCategoryRequired, client);
		Format(sTalentInformation, sizeof(sTalentInformation), "%T", "category level required", client, iTalentsInCategory, iTalentLevelRequired);
	for (new i = 0; i < iTalentsRequired; i++) {

		ExplodeString(tTalentName[i], "+", tTalentNameInfo, 2, 64);
		// This way we only show the talent requirement(s) that the player is missing.
		if (GetTalentStrength(client, tTalentNameInfo[0]) < StringToInt(tTalentNameInfo[1])) {

			Format(tTranslation, sizeof(tTranslation), "%T", tTalentNameInfo[0], client);
			if (StrEqual(sTalentInformation, "-1")) Format(sTalentInformation, sizeof(sTalentInformation), "%s: %s", tTranslation, tTalentNameInfo[1]);
			else Format(sTalentInformation, sizeof(sTalentInformation), "%s\n%s: %s", sTalentInformation, tTranslation, tTalentNameInfo[1]);
		}
	}
	return sTalentInformation;
}
...
public BuildSubMenuHandle(Handle:menu, MenuAction:action, client, slot)
{
	if (action == MenuAction_Select)
	{
		...
		for (new i = 0; i < size; i++) {
			...
			if (isSpecialAmmo == 1 && bEquipSpells[client]) {
		...
		if (isSubMenu == 1 || isSubMenu == 2) {
			...
			else bEquipSpells[client] = false;
			...
			if (ClassRole != 1 && AbilityTalent != 1 && IsTalentLocked(client, TalentName) && SkyPoints[client] >= StorePurchaseCost) {
				...
				if (StrEqual(sTalentRequirements, "-1") && (AbilityTalent == 1 || PlayerLevel[client] >= TalentLevelRequired || bEquipSpells[client])) {// submenu 2 is to send to spell equip screen *Flex*

					PurchaseTalentName[client] = TalentName;
					PurchaseSurvEffects[client] = SurvEffects;
					PurchaseTalentPoints[client] = PlayerTalentPoints;

					if (!bEquipSpells[client]) ShowTalentInfoScreen(client, TalentName, MenuKeys[client], MenuValues[client]);
					else ShowTalentInfoScreen(client, TalentName, MenuKeys[client], MenuValues[client], true);
				}
				else {
...
stock ShowTalentInfoScreen(client, String:TalentName[], Handle:Keys, Handle:Values, bool:bIsEquipSpells = false) {
	...
	if (IsAbilityType == 1 || bIsEquipSpells) SendPanelToClientAndClose(TalentInfoScreen_Special(client), client, TalentInfoScreen_Special_Init, MENU_TIME_FOREVER);
...
stock Float:GetTalentInfo(client, Handle:Keys, Handle:Values, infotype = 0, bool:bIsNext = false, String:TalentNameOverride[] = "none", target = 0, iStrengthOverride = 0) {
	...
	new Float:f_StrEach = f_Strength - 1.0;
	new Float:TheAbilityMultiplier = 0.0;
	...
	if (infotype == 0) {	// first point value
		...
		if (target == client) f_StrengthT = (f_StrengthPoint * (ConsMult * Cons));
		...
		f_StrengthT				= (f_StrengthPoint * (AgilMult * Agil));
		
		if (target == client) f_StrengthPoint += (f_StrengthPoint * f_StrengthT);
		...
		f_StrengthT				= (f_StrengthPoint * (TechMult * Tech));
		...
		if (f_StrEach > 0.0) {
			...
			f_StrengthFirst = GetKeyValueFloat(Keys, Values, "ability time first point?");
			if (f_StrengthFirst > 0.0) f_StrengthPoint += f_StrengthFirst;
		}
	}
		...
		f_StrengthFirst = GetClassMultiplier(target, f_StrengthFirst, "ftCD");
		f_StrengthPoint = GetClassMultiplier(target, f_StrengthPoint, "etCD");

		if (target == client) {

			f_StrengthFirst			+= (f_StrengthFirst * (ConsMult * Cons));
			f_StrengthPoint			+= (f_StrengthPoint * (AgilMult * Agil));
		}

		f_StrengthT				= (f_StrengthFirst * (ResiMult * Resi));
		f_StrengthFirst			-= f_StrengthT;
		if (f_StrengthFirst < 0.0) f_StrengthFirst = 0.0;
		f_StrengthT				= (f_StrengthPoint * (EnduMult * Endu));
		f_StrengthPoint			-= f_StrengthT;
		if (f_StrengthPoint < 0.0) f_StrengthPoint = 0.0;
	...
	if (StrEqual(TalentNameOverride, "none")) {
		...
		f_StrengthPoint = GetClassMultiplier(target, f_StrengthPoint, talenteffects);
	}
...
public Handle:TalentInfoScreen (client) {
	...
	else if (iIsClassesEnabled == 1 && ClassRole == 1 && !bIsInCombat[client] && !IsLoadingClassData[client]) {
...
public TalentInfoScreen_Init (Handle:topmenu, MenuAction:action, client, param2)
{
	if (action == MenuAction_Select)
	{
		...
		new bool:bIsNotEligible = false;
		if (ClassRole != 1 && !TalentRequirementsMet(client, sTalentsRequired)) bIsNotEligible = true;
					...
					if (!bIsNotEligible) {
						...
						if (ClassRole != 1 && AbilityTalent != 1) {
							...
							if (IsLegitimateClass(client)) {
								...
								if (!StrEqual(ActiveClass[client], TalentName, false)) Format(ClassLoadQueue[client], sizeof(ClassLoadQueue[]), "%s", TalentName);
								else Format(ClassLoadQueue[client], sizeof(ClassLoadQueue[]), "vanilla");
								SaveClassData(client);
							}
							else {
