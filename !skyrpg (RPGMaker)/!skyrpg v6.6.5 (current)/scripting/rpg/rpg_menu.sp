stock BuildMenuTitle(client, Handle:menu, bot = 0, type = 0, bool:bIsPanel = false) {	// 0 is legacy type that appeared on all menus. 0 - Main Menu | 1 - Upgrades | 2 - Points
	...
	if (bot == 0) {
		...
		if (CheckRPGMode > 0) {
			...
			Format(PlayerLevelText, sizeof(PlayerLevelText), "%T", "Player Level Text", client, PlayerLevel[client], iMaxLevel, SkyLevel[client], iSkyLevelMax, AddCommasToString(ExperienceLevel[client]), MenuExperienceBar(client), AddCommasToString(CheckExperienceRequirement(client)));
...
stock LoadProfileEx(client, String:key[]) {
	...
	if (IsSurvivorBot(LoadTarget[client]) || IsSurvivorBot(client) || LoadTarget[client] == -1 || IsLegitimateClient(LoadTarget[client]) && GetClientTeam(LoadTarget[client]) == TEAM_SURVIVOR) {
		...
		if (IsSurvivorBot(targetClient) || !IsLoadingClassData[targetClient]) {
...
stock GetTeamComposition(client) {
	...
	for (new i = 1; i <= MaxClients; i++) {
		...
		if (!IsLegitimateClass(i)) Format(Classname, sizeof(Classname), "Vanilla");
...
stock BuildMenu(client, String:TheMenuName[] = "none") {
	...
	for (new i = 0; i < size; i++) {
		...
		if (iIsReadMenuName == 1) {
			...
			else if (StrEqual(configname, "noob toggle")) {

				if (iNoobAssistance[client] == 0 || SkyLevel[client] > 0 || PlayerLevel[client] > iNoobAssistanceLevel) Format(text, sizeof(text), "%T", "noob mode (disabled)", client);
				else Format(text, sizeof(text), "%T", "noob mode (enabled)", client);
			}
...
public BuildMenuHandle(Handle:menu, MenuAction:action, client, slot) {
	...
	if (action == MenuAction_Select) {
		...
		else if (StrEqual(config, "noob toggle", false)) {

			if (SkyLevel[client] == 0 && PlayerLevel[client] <= iNoobAssistanceLevel && iNoobAssistance[client] == 0) {

				iNoobAssistance[client] = 1;
			}
			else iNoobAssistance[client] = 0;
			BuildMenu(client);
		}
...
stock LoadProficiencyData(client) {
	...
	decl String:text[64];
	for (new i = 0; i <= 7; i++) {
		new CurLevel = GetProficiencyData(client, i);
		new CurExp = GetProficiencyData(client, i, _, 1);
		new CurGoal = GetProficiencyData(client, i, _, 2);
		...
		Format(text, sizeof(text), "%s Lv.%d %d%s%dXP", text, CurLevel, CurExp, MenuExperienceBar(client, CurExp, CurGoal), CurGoal);
...
stock BuildSubMenu(client, String:MenuName[], String:ConfigName[], String:ReturnMenu[] = "none") {
	...
	for (new i = 0; i < size; i++) {
		...
		if (isSubMenu == 1 || isSubMenu == 2) {
			...
			if (SkyLevel[client] < iSkyLevelReq) continue;
			...
			else if (ClassRole != 1 && AbilityTalent != 1) {
		...
		if (iSkyLevelReq > 0) Format(text, sizeof(text), "%s (Req SLv.%d+)", text, iSkyLevelReq);
...
public BuildSubMenuHandle(Handle:menu, MenuAction:action, client, slot)
{
	if (action == MenuAction_Select)
	{
		...
		for (new i = 0; i < size; i++) {
			...
			if (ClassRole != 1 && isSubMenu != 1 && isSubMenu != 2) {
...
public Handle:TalentInfoScreen (client) {
	...
	if (ClassRole != 1 && AbilityTalent != 1) {
		...
		TalentPointMaximum		= GetKeyValueInt(PurchaseKeys[client], PurchaseValues[client], "maximum talent points allowed?");
	}
	new TalentType = GetKeyValueInt(PurchaseKeys[client], PurchaseValues[client], "talent type?");

	new Float:s_FirstPoint = GetTalentInfo(client, PurchaseKeys[client], PurchaseValues[client]);
	new Float:s_OtherPoint = GetTalentInfo(client, PurchaseKeys[client], PurchaseValues[client], 1);
	new Float:s_OtherPointNext = GetTalentInfo(client, PurchaseKeys[client], PurchaseValues[client], 1, true);
	...
	if (ClassRole != 1 && AbilityTalent != 1) {
		...
		if (IsSpecialAmmo != 1) {
			...
			new AbilityType = StringToInt(GetKeyValue(PurchaseKeys[client], PurchaseValues[client], "ability type?"));
			...
			if (GetKeyValueInt(PurchaseKeys[client], PurchaseValues[client], "talent type?") <= 0) {
	...
	else if (ClassRole == 1 && !IsLoadingClassData[client]) {
		// !bIsInCombat[client] &&
	...
	if (ClassRole != 1) {
		...
		if (!bIsClassAbilities[client]) DrawPanelText(menu, GetClassMultiplierText(client, GetKeyValue(PurchaseKeys[client], PurchaseValues[client], "class effect?"), TalentName));
