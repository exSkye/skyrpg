...
public DBConnect(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	if (GenerateDB == 1) {
		...
		SQL_TQuery(hDatabase, QueryResults, tquery);
		Format(tquery, sizeof(tquery), "ALTER TABLE `%s` ADD `noobass` int(32) NOT NULL DEFAULT '1';", TheDBPrefix);
		SQL_TQuery(hDatabase, QueryResults, tquery);
...
stock ClearAndLoad(client, bool:IgnoreLoad = false) {
	...
	Format(tquery, sizeof(tquery), "SELECT `steam_id`, `exp`, `expov`, `upgrade cost`, `level`, `skylevel`, `%s`, `time played`, `talent points`, `total upgrades`, `free upgrades`, `restt`,`restexp`, `lpl`, `resr`, `survpoints`, `bec`, `rem`, `pri`, `tcolour`, `tname`, `ccolour`, `xpdebt`, `upav`, `upawarded`, `%s`, `aclass`, `myrating %s`, `ratinghc %s`, `lastserver`, `myseason`, `lvlpaused`, `itrails`, `noobass`, `pistol_xp`, `melee_xp`, `uzi_xp`, `shotgun_xp`, `sniper_xp`, `assault_xp`, `medic_xp`, `grenade_xp` FROM `%s` WHERE (`steam_id` = '%s');", themenu, RatingType, RatingType, RatingType, TheDBPrefix, key);
...
public Query_SaveClassData(Handle:owner, Handle:hndl, const String:error[], any:client) {
	...
	if (!IsLegitimateClient(client) || iIsClassesEnabled == 1 && !IsLegitimateClass(client)) return;
	...
	if (!IsSurvivorBot(client)) {
		...
		Format(tquery, sizeof(tquery), "%s, `secondwep` = '%s' WHERE (`steam_id` = '%s%s%s');", tquery, text, key, ActiveClass[client], CLASS_VERSION);
		SQL_TQuery(hDatabase, QueryResults, tquery);
	}
	...
	if (StrEqual(ClassLoadQueue[client], "vanilla")) {
...
stock SaveAndClear(client, bool:b_IsTrueDisconnect = false, bool:IsNewPlayer = false) {
	...
	Format(tquery, sizeof(tquery), "UPDATE `%s` SET `tcolour` = '%s', `tname` = '%s', `ccolour` = '%s', `noobass` = '%d' WHERE (`steam_id` = '%s');", TheDBPrefix, TagColour, TagName, ChatColour, iNoobAssistance[client], key);
...
public QueryResults_Load(Handle:owner, Handle:hndl, const String:error[], any:client)
{
	if ( hndl != INVALID_HANDLE )
	{
		...
		while (SQL_FetchRow(hndl))
		{
			...
			SQL_FetchString(hndl, 26, ActiveClass[client], sizeof(ActiveClass[]));
			Rating[client] = SQL_FetchInt(hndl, 27);
			RatingHandicap[client] = SQL_FetchInt(hndl, 28);
			SQL_FetchString(hndl, 29, t_Hostname, sizeof(t_Hostname));
			...
			SQL_FetchString(hndl, 30, CurrentSeason, sizeof(CurrentSeason));
			iIsLevelingPaused[client]	= SQL_FetchInt(hndl, 31);
			iIsBulletTrails[client]		= SQL_FetchInt(hndl, 32);
			iNoobAssistance[client]		= SQL_FetchInt(hndl, 33);

			pistolXP[client] = SQL_FetchInt(hndl, 34);
			meleeXP[client] = SQL_FetchInt(hndl, 35);
			uziXP[client] = SQL_FetchInt(hndl, 36);
			shotgunXP[client] = SQL_FetchInt(hndl, 37);
			sniperXP[client] = SQL_FetchInt(hndl, 38);
			assaultXP[client] = SQL_FetchInt(hndl, 39);
			medicXP[client] = SQL_FetchInt(hndl, 40);
			grenadeXP[client] = SQL_FetchInt(hndl, 41);
		}
		if (PlayerLevel[client] > 0) {
			...
			if (!IsFakeClient(client) && !IsLegitimateClass(client)) AwardExperience(client, -1);
...
public Action:Timer_ClassLoadCheck(Handle:timer, any:client) {
	...
	if (!IsLoadingClassData[client] && IsPlayerAlive(client) && (GetClientTeam(client) == TEAM_SURVIVOR || IsSurvivorBot(client))) {//if (!b_IsLoadingTrees[client]) {
		...
		if (bIsTalentTwo[client]) {
			...
			if (!IsLegitimateClass(client)) return Plugin_Stop;
			...
			if (!IsFakeClient(client)) {
				...
				PrintToChat(client, "%T", "class profile loaded", client, orange, blue, TheName);
				AwardExperience(client, -1);

				//bIsGiveProfileItems[client] = true;
				GiveProfileItems(client);
				...
				else if (RatingHandicap[client] > iMaxHandicap) RatingHandicap[client] = iMaxHandicap;

				if (iIsRatingEnabled == 1) {
