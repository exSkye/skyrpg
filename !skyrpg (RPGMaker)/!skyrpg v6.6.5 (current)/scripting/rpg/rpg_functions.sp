...
stock GetBaseWeaponDamage(client, target, Float:impactX = 0.0, Float:impactY = 0.0, Float:impactZ = 0.0, damagetype, bool:bGetBaseDamage = false) {
	...
	for (new i = 0; i < size; i++) {
		...
		if (StrContains(Weapon, "rifle", false) != -1 && StrContains(Weapon, "hunting", false) == -1) WeaponDamage = RoundToCeil(GetClassMultiplier(client, WeaponDamage * 1.0, "riflDMG"));
...
stock GetProficiencyData(client, proficiencyType = 0, iExperienceEarned = 0, iReturnType = 0, bGetLevel = false) {
	...
	*/
	...
	while (proficiencyExperience >= experienceRequirement) {
		...
		proficiencyLevel++;
		experienceRequirement = RoundToCeil(iProficiencyStart * (fProficiencyExperienceMultiplier * proficiencyLevel));
	}
	if (bGetLevel || iReturnType == 0) return proficiencyLevel;
	if (iReturnType == 1) return proficiencyExperience;
	if (iReturnType == 2) return experienceRequirement;
	if (iExperienceEarned > 0 && curLevel < proficiencyLevel) {
