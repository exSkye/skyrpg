...
stock LoadMainConfig() {
	...
	iIsClassesEnabled			= GetConfigValueInt("Is Classes Enabled?");
...
stock DeleteAndCreateNewData(client, bool:IsBot = false) {
	...
	if (!IsBot) {
		...
		for (new i = 0; i < size; i++) {
			...
			Format(tquery, sizeof(tquery), "DELETE FROM `%s` WHERE `steam_id` LIKE '%s%s%s%s';", TheDBPrefix, pct, key, text, pct);
			SQL_TQuery(hDatabase, QueryResults, tquery, client);
		}
		ResetData(client);
