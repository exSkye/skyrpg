stock BuildDirectorPriorityMenu(client) {
	...
	for (new i = 0; i < size; i++) {
		...
		Format(Name_t, sizeof(Name_t), "%s (%d / %d)", Name_t, Priority, StringToInt(GetConfigValue("director priority maximum?")));
...
public BuildDirectorPriorityMenuHandle(Handle:menu, MenuAction:action, client, slot) {
	...
	if (action == MenuAction_Select) {
		...
		new PriorityMaximum				=	StringToInt(GetConfigValue("director priority maximum?"));
