...
public Handle UnequipAugment_Compare(int client) {
	...
	int iEquippedAugmentLevel = (GetArrayCell(myAugmentInfo[client], augmentSlotToEquipOn[client]) + GetArrayCell(myAugmentInfo[client], augmentSlotToEquipOn[client], 4) + GetArrayCell(myAugmentInfo[client], augmentSlotToEquipOn[client], 5)) / iAugmentLevelDivisor;
	...
	if (!StrEqual(augmentActivator, "-1")) DrawPanelText(menu, augmentActivator);
	if (!StrEqual(augmentTarget, "-1")) DrawPanelText(menu, augmentTarget);
	int iInspectedAugmentLevel = (GetArrayCell(myAugmentInfo[client], AugmentClientIsInspecting[client]) + GetArrayCell(myAugmentInfo[client], AugmentClientIsInspecting[client], 4) + GetArrayCell(myAugmentInfo[client], AugmentClientIsInspecting[client], 5)) / iAugmentLevelDivisor;
...
stock void Reroll_Augment_Pay(int client) {
	...
	Format(text, sizeof(text), "Materials: %s\n \n%s\n%s", scrap, augmentName, augmentCategory);
	if (!StrEqual(augmentActivator, "-1")) Format(text, sizeof(text), "%s\n%s", text, augmentActivator);
	if (!StrEqual(augmentTarget, "-1")) Format(text, sizeof(text), "%s\n%s", text, augmentTarget);
	...
	Format(text, sizeof(text), "Confirm Reroll for %d Materials", ((type == 0) ? iAugmentCategoryRerollCost : (type == 1) ? iAugmentActivatorRerollCost : iAugmentTargetRerollCost));
...
public Handle Reroll_Augment(client) {
	...
	if (!StrEqual(augmentTarget, "-1")) DrawPanelText(menu, augmentTarget);
	DrawPanelText(menu, "\n \n");
	Format(text, sizeof(text), "(%d Materials) Reroll %s", iAugmentCategoryRerollCost, augmentCategoryName);
	DrawPanelItem(menu, text);

	if (!StrEqual(augmentActivator, "-1") && GetArraySize(myUnlockedActivators[client]) > 1) {
		Format(text, sizeof(text), "(%d Materials) Reroll %s", iAugmentActivatorRerollCost, augmentActivatorName);
		DrawPanelItem(menu, text);
	}
	if (!StrEqual(augmentTarget, "-1") && GetArraySize(myUnlockedTargets[client]) > 1) {
		Format(text, sizeof(text), "(%d Materials) Reroll %s", iAugmentTargetRerollCost, augmentTargetName);
		DrawPanelItem(menu, text);
	}
	Format(text, sizeof(text), "%T", "return to talent menu", client);
	DrawPanelItem(menu, text);
...
public Handle Upgrade_Augment(client) {
	...
	if (currentCategoryScoreRoll < maxCategoryScoreRoll || currentActivatorScoreRoll > 0 && currentActivatorScoreRoll < maxActivatorScoreRoll || currentTargetScoreRoll > 0 && currentTargetScoreRoll < maxTargetScoreRoll) {
		...
		Format(pct, sizeof(pct), "%");
		if (currentCategoryScoreRoll < maxCategoryScoreRoll) {
			Format(text, sizeof(text), "(%d Materials) Upgrade %s", iAugmentCategoryUpgradeCost, augmentCategoryName);
			DrawPanelItem(menu, text);
		...
		if (currentActivatorScoreRoll > 0 && currentActivatorScoreRoll < maxActivatorScoreRoll) {
			Format(text, sizeof(text), "(%d Materials) Upgrade %s", iAugmentActivatorUpgradeCost, augmentActivatorName);
			DrawPanelItem(menu, text);
		...
		if (currentTargetScoreRoll > 0 && currentTargetScoreRoll < maxTargetScoreRoll) {
			Format(text, sizeof(text), "(%d Materials) Upgrade %s", iAugmentTargetUpgradeCost, augmentTargetName);
			DrawPanelItem(menu, text);
