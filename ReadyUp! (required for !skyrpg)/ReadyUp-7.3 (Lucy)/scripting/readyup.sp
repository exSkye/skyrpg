...
#define PLUGIN_VERSION		"7.3 Lucy"
...
public ReadyUp_Command(client, String:command[]) {
	...
	else if (StrEqual(command, s_Cmd_ToggleBots) && t_SetBotTimer != INVALID_HANDLE) Cmd_ToggleBots(client);
}
...
public Action:Event_MapTransition(Handle:event, String:event_name[], bool:dontBroadcast) {
	...
	if (GetGamemodeType() != 2 && !b_IsRoundOver) {
		...
		LogMessage("The map is changing");
		//ResetMapVars();
...
public OnMapStart() {
	...
	/*if (!IsEligibleMap(0) && i_IsFreeze == 0 && i_IsReadyUpIgnored == 0) {
		...
		//SaferoomDoor						= Now_FindAndLockSaferoomDoor();
		ToggleSaferoomDoor(true);	// true to lock.
	}*/
}
...
public Action:Timer_SetBotTimer(Handle:timer, Handle:thepackage) {
	...
	if (CountdownTimer < 1) {
		...
		else PrintToChatAll("%t", "adjust bot count", blue, white, green, iCount, white, orange, CountdownTimer);
	}
	return Plugin_Continue;
}
...
public Action:Timer_IsFreeze(Handle:timer) {
	...
	for (new i = 1; i <= MaxClients; i++) {
		...
		if (b_IsReadyUp || !b_IsReadyUpOver) {
...
stock ToggleSaferoomDoor(bool:UnlockDoor) {
	...
	while ((ent = FindEntityByClassname(ent, "prop_door_rotating_checkpoint")) != -1) {
		...
		//if (!IsValidEntity(ent)) continue;
		/*if (bool:GetEntData(ent, OFFSET_LOCKED, 1) && bIsUnlock) {
		...
		else if (SaferoomDoor >= 0 && UnlockDoor && IsValidEntity(SaferoomDoor)) {
