...
#define PLUGIN_VERSION		"7.4 Copenhagen"
#define PLUGIN_CONTACT		"github.com/iammyka"
...
stock GetCurrentCampaignName() {
	...
	if (iForceCampaign == 1) {
		...
		*/
		new size = GetArraySize(Handle:a_FirstMap) - 1;
...
stock ToggleSaferoomDoor(bool:UnlockDoor) {
	...
	while ((ent = FindEntityByClassname(ent, "prop_door_rotating_checkpoint")) != -1) {
		...
		else if (SaferoomDoor >= 0 && UnlockDoor) {
