...
public ReadyUp_TrueDisconnect(client) { CreateTimer(3.0, Timer_ManageSlots, _, TIMER_FLAG_NO_MAPCHANGE); }
...
public Now_ManageSlots() {
	...
	new SpecSlots = GetSpectatorCount();

	//new MaxSlots = GetHumanCount();
	new MaxSlots = i_PlayerSlots;
	new MaximumAllowedSlots = i_PlayerSlots + i_ReserveSlots + SpecSlots;

	if (MaxSlots < i_PlayerSlots) SetConVarInt(FindConVar("sv_maxplayers"), i_PlayerSlots + SpecSlots + i_ReserveSlots + 1);
	else SetConVarInt(FindConVar("sv_maxplayers"), i_PlayerSlots + SpecSlots + i_ReserveSlots);
	SetConVarInt(FindConVar("sv_visiblemaxplayers"), MaximumAllowedSlots);
	...
	if (i_SurvivorSlots < 0 || i_InfectedSlots < 0) {
		...
		SetConVarBounds(FindConVar("z_max_player_zombies"), ConVarBound_Upper, true, (i_PlayerSlots / 2) * 1.0);
		SetConVarInt(FindConVar("z_max_player_zombies"), i_PlayerSlots / 2);
		SetConVarBounds(FindConVar("survivor_limit"), ConVarBound_Upper, true, MaxSlots * 1.0);
		SetConVarInt(FindConVar("survivor_limit"), MaxSlots);
	}
	else {
