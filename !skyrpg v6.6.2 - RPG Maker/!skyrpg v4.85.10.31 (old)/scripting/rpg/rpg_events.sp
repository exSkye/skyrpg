		...
		if (b_IsFinaleActive) {
			...
			for (new i = 1; i <= MaxClients; i++) {
				...
				if (IsLegitimateClientAlive(i) && (GetClientTeam(i) == TEAM_SURVIVOR || IsSurvivorBot(i))) {
					...
					if (RoundExperienceMultiplier[i] > 0.0 && BonusContainer[i] > 0) {
						...
						BonusContainer[i] += RoundToCeil(BonusContainer[i] * RoundExperienceMultiplier[i]);
						ExperienceLevel[i] += BonusContainer[i];
						ExperienceOverall[i] += BonusContainer[i];
						BonusContainer[i] = 0;
						RoundExperienceMultiplier[i] = 0.0;
						ConfirmExperienceAction(i);
					}
				}
			}
		}
		...
		if (IsFakeClient(attacker)) {
			...
			if (FindZombieClass(attacker) == ZOMBIECLASS_TANK) {
				...
				if (b_IsFinaleActive && b_IsFinaleTanks) {
					...
					for (new i = 0; i + iTankCount < iSurvivors; i++) {
			...
			if (iTankRush == 1) {
				...
				if (FindZombieClass(attacker) != ZOMBIECLASS_TANK) {
					...
					if (!b_IsFinaleActive && iSurvivors >= 1 && iTankCount < (iSurvivors + iSurvivorBots)) {

						if (IsLegitimateClientAlive(theClient)) ExecCheatCommand(theClient, "z_spawn_old", "tank auto");
					}
				}
			}
			else if (FindZombieClass(attacker) != ZOMBIECLASS_TANK) {

				new iEnsnaredCount = EnsnaredInfected();
				if (IsEnsnarer(attacker)) {

					if (iInfectedLimit == -1 || iInfectedLimit > 0 && iEnsnaredCount >= iInfectedLimit) {
...
stock bool:UseAbility(client, target = -1, String:TalentName[], Handle:Keys, Handle:Values, Float:TargetPos[3]) {
		...
		if (IsValidEntity(MySecondary)) {
			...
			if (StrContains(MyWeapon, "pistol", false) != -1) {
				...
				if (StrContains(MyWeapon, "magnum", false) == -1) {
					...
					CreateTimer(0.01, Timer_GiveSecondPistol, client, TIMER_FLAG_NO_MAPCHANGE);
				}
			}
		}
	}
	...
	if (StrContains(Effects, "S", true) != -1) {
		...
		if (IsLegitimateClientAlive(MyAttacker)) L4D_StaggerPlayer(MyAttacker, client, NULL_VECTOR);
...
stock CalculateInfectedDamageAward(client, killerblow = 0) {
	...
	new Float:TheAbilityMultiplier = 0.0;
	new Float:SurvivorRushingMult = 0.0;

	if (IsLegitimateClientAlive(killerblow) && (GetClientTeam(killerblow) == TEAM_SURVIVOR || IsSurvivorBot(killerblow))) {
	...
	for (new i = 1; i <= MaxClients; i++) {
		...
		SurvivorRushingMult = SurvivorRushingMultiplier(i);
		RatingBonus = RoundToCeil(RatingBonus * SurvivorRushingMult);
		if (RatingBonus > 0) {

			if (!IsFakeClient(i) && (IsSpecialCommon(client) || !IsCommonInfected(client))) PrintToChat(i, "%T", "rating increase", i, white, blue, AddCommasToString(RatingBonus), orange);
			CheckMinimumRate(i);
		...
		HealingContribution[i] += RoundToCeil(h_Contribution * SurvivorRushingMult);
		TankingContribution[i] += RoundToCeil(t_Contribution * SurvivorRushingMult);
		PointsContribution[i] += (SurvivorPoints * SurvivorRushingMult);
		DamageContribution[i] += RoundToCeil(SurvivorExperience * SurvivorRushingMult);
...
stock ReceiveInfectedDamageAward(client, infected, e_reward, Float:p_reward, t_reward, h_reward , bu_reward, he_reward, bool:TheRoundHasEnded = false) {
	...
	new Float:TeammateBonus = (LivingSurvivors() - 1) * fSurvivorExpMult;
	...
	new Float:SurvivorRushingMult = SurvivorRushingMultiplier(client);
	...
	if (RPGMode >= 0 && RPGMode != 1 && p_reward > 0.0) {
		...
		Points[client] += (p_reward * SurvivorRushingMult);
	...
	if (IsLegitimateClientAlive(target) && IsLegitimateClientAlive(healer) && GetClientTeam(target) == GetClientTeam(healer)) {
		...
		if (!HealImmunity[target]) { // && !IsBeingRevived(target)) {
	 		...
	 		if (!TheBool || !bIsMeleeCooldown[healer]) {
		 		...
		 		if (!TheBool) HealerAmount = GetClassMultiplier(healer, iHealerAmount * 1.0, "hB", _, true);
		 		else HealerAmount = GetClassMultiplier(healer, iHealerAmount * 1.0, "hM", _, true);
