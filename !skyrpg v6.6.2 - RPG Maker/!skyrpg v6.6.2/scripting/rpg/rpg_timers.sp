		...
		if (IsLegitimateClientAlive(i) && GetClientTeam(i) != TEAM_SPECTATOR) {
			...
			if ((GetClientTeam(i) == TEAM_SURVIVOR || IsSurvivorBot(i)) && CurrentRPGMode >= 1) {
				...
				if (ThisRoundTime < EnrageCount && L4D2_GetInfectedAttacker(i) == -1) {
					...
					if (!IsSurvivorBot(i) && iNoobAssistance[i] == 1) {

						if (!IsIncapacitated(i)) healregenamount = (mymaxhealth * fNoobAssistanceHealing);
						else if (!b_IsFinaleActive) healregenamount = (mymaxhealth * fNoobAssistanceRecovery);
...
public Action:Timer_DirectorPurchaseTimer(Handle:timer) {
	...
	if (iTanksAlways >= 0) {
		...
		if (theTankStartTime == 0 || RPGRoundTime(true) >= theTankStartTime) {
			...
			if (!b_IsFinaleActive && ((iTanksAlways > 0 && iTankCount < iTankLimit + iTanksAlways) || (iTanksAlways == 0 && iTankCount < iSurvivors + iSurvivorBots))) {
