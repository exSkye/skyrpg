		...
		if (TalentStrength > 0) {
			...
			if (bIsAbility) {
				...
				if (fAmmoCooldownTime != -1.0) {
					...
					fAmmoCooldown = GetAbilityValue(client, talentname, "cooldown?");
...
stock Float:CheckActiveAbility(client, thevalue, eventtype = 0, bool:IsPassive = false) {
	...
	if (IsSurvivorBot(client)) return 0.0;
...
public Handle:TalentInfoScreen (client) {
	...
	else if (ClassRole == 1 && !bIsInCombat[client] && !IsLoadingClassData[client]) {
			...
			switch (param2)
			{
				case 1: {
					...
					if (ClassRole != 1 && AbilityTalent != 1) {
						...
						if (IsLegitimateClass(client)) SaveClassData(client);
