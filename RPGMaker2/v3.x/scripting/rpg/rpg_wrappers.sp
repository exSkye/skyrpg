...
stock int GetBaseWeaponDamage(int client, int target, float impactX = 0.0, float impactY = 0.0, float impactZ = 0.0, int damagetype, bool bGetBaseDamage = false, bool IsDataSheet = false, int hitgroup = -1) {
	...
	for (int i = 0; i < size; i++) {
		...
		float mod = GetTalentModifier(client, MODIFIER_DAMAGE);
		if (mod != 0.0) WeaponDamage += RoundToCeil(WeaponDamage * mod);
...
public Action OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage_ignore, int &damagetype) {
	...
	float AltAbilityMultiplier = 0.0;
	int cTank = -1;
		...
		if (victimTeam == TEAM_SURVIVOR) {
			...
			if (attackerType <= 3) {
				...
				if (attackerType == 3) {
					...
					int damageReduction = RoundToCeil(GetAbilityStrengthByTrigger(victim, attacker, "L", _, i_WitchDamage, _, _, "o", 2, true));
					...
					// Reflect damage.
					ammoStr = IsClientInRangeSpecialAmmo(victim, "R", false, _, i_WitchDamage * 1.0);
...
stock int IfCommonInfectedIsAttackerDoStuff(attacker, victim, damagetype, survivorCount) {
	...
	if (!(damagetype & DMG_DIRECT)) {
		...
		float AltAbilityMultiplier = GetAbilityMultiplier(victim, "expo");
		...
		int damageReduction = RoundToCeil(GetAbilityStrengthByTrigger(victim, attacker, "L", _, CommonsDamage, _, _, "o", 2, true));
		if (damageReduction > 0) {
		...
		GetAbilityStrengthByTrigger(victim, attacker, "L", _, CommonsDamage + ((damageReduction > 0) ? damageReduction : 0));	// we do not penalize players who stack damage reduction.
	}
	...
	int BuffDamage = 0;
	ammoStr = IsClientInRangeSpecialAmmo(victim, "R", false, _, CommonsDamage * 1.0);
...
stock int IfInfectedIsAttackerDoStuff(attacker, victim) {
	...
	// float GetAbilityStrengthByTrigger(activator, targetPlayer = 0, char[] AbilityT, zombieclass = 0, damagevalue = 0,
	...
	int damageReduction = RoundToCeil(GetAbilityStrengthByTrigger(victim, attacker, "L", _, totalIncomingDamage, _, _, "o", 2, true));
	if (damageReduction > 0) {
	...
	int ReflectIncomingDamage = 0;
	ammoStr = IsClientInRangeSpecialAmmo(victim, "R", false, _, totalIncomingDamage * 1.0);
...
stock AddSpecialInfectedDamage(client, target, TotalDamage = 0, bool IsTankingInstead = false, damagevariant = -1, ammotype = -1, hitgroup = -1) {
	...
	if (!IsTankingInstead) {
		...
		int i_HealthRemaining = RoundToCeil(i_InfectedMaxHealth * (1.0 - CheckTeammateDamages(target, client)));
		...
		if (damagevariant != 2 && i_DamageBonus > 0) {
			...
			/*if (damagevariant == 1) AddTalentExperience(client, "endurance", i_DamageBonus);
				...
				bool bIsMeleeAttack = IsMeleeAttacker(client);
...
stock AddSpecialCommonDamage(client, entity, playerDamage, bool IsStatusDamage = false, damagevariant = -1, ammotype = -1, hitgroup = -1) {
	...
	if (playerDamage >= 0) {
		...
		maxHP = RoundToCeil(maxHP * (1.0 - CheckTeammateDamages(entity, client)));
...
stock float GetInfectedAbilityStrengthByTrigger(activator, targetPlayer = 0, char[] AbilityT, zombieclass = 0, damagevalue = 0,
	...
	for (int i = 0; i < ASize; i++) {
		...
		if (activatorClassesAllowed != -1 && !clientClassIsAllowed(activator, activatorClassesAllowed, activatorClass)) continue;
		...
		if (bIsCompounding) {
			...
			if (!IsOverdriveStacks) {
				...
				if (!IsCleanse || GetArrayCell(TriggerValues[activator], IS_OWN_TALENT) == 1) {
					...
					if (ResultType >= 1) {
						if (!bDontActuallyActivate) {
							// fPercentageHealthRequired = GetArrayCell(TriggerValues[activator], HEALTH_PERCENTAGE_ACTIVATION_COST);
							// emaining < fPercentageHealthRequired) continue;
							...
							ActivateAbilityEx(activator, target, damagevalue, targeteffects, p_Strength, p_Time, target, _, isRawType,
												...
												abilityTrigger, damagetype, _, activatorCallAbilityTrigger, entityIdToPassThrough, fPercentageHealthActivationCost, targetCallAbilityTrigger);
						}
					}
					else {
						if (!bDontActuallyActivate) {
...
stock float GetAbilityStrengthByTrigger(activator, targetPlayer = 0, char[] AbilityT, zombieclass = 0, damagevalue = 0,
	...
	int targetClass = (isTargetLegitimate) ? (GetClientTeam(targetPlayer) == TEAM_SURVIVOR) ? 0 : FindZombieClass(targetPlayer) : (IsWitch(targetPlayer)) ? 7 : (IsCommonInfected(targetPlayer)) ? 9 : -1;
	...
	for (int i = 0; i < ASize; i++) {
		...
		if ((targetClassesAllowed != -1 || !bIsEffectOverTimeIgnoresClass) && !clientClassIsAllowed(activator, targetClassesAllowed, targetClass)) continue;
		...
		*/
		...
		if (multStrengthByNearbyAllies > 0.0) {
			float multStrengthByNearbyAlliesRange = GetArrayCell(TriggerValues[activator], MULT_STR_NEARBY_DOWN_ALLIES_RANGE);
			int numOfAlliesInRange = GetClientsInRangeByState(activator, multStrengthByNearbyAlliesRange, true, _, SURVIVOR_STATE_INCAPACITATED);
			if (numOfAlliesInRange > 0) f_Strength *= (numOfAlliesInRange * multStrengthByNearbyAllies);
		}
...
stock int GetLayerUpgradeStrength(client, layer = 1, bool bIsCheckEligibility = false, bool bResetLayer = false,
	...
	for (int i = 0; i < size; i++) {										// loop through this talents keyfile.
		...
		char mytalentname[64];
		GetArrayString(Section, 0, mytalentname, 64);
		//PrintToChat(client, "Talent name: %s", mytalentname);
		int activatorClassesAllowed = GetArrayCell(GetLayerStrengthValues[client], ACTIVATOR_CLASS_REQ);
		//PrintToChat(client, "classes allowed: %d", activatorClassesAllowed);
		if (activatorClassesAllowed > -1) {
			...
			//PrintToChat(client, "%s, %d, %d %d", mytalentname, activatorClassesAllowed, isSurvivorTalent, isInfectedTalent);
	...
	*/
	...
	if (g_TalentStrength > 0.0) {
		...
		if (StrEqual(Effects, "vampire")) {
			...
			CreateAoE(target, AoERange, (g_TalentStrength < 1.0) ? RoundToCeil(d_Damage * g_TalentStrength) : RoundToCeil(g_TalentStrength), _, _, hitgroup, damagetype, targetCallAbilityTrigger);
		}
		else if (StrEqual(Effects, "ffexplode")) {
			CreatePlayerExplosion(target, 384.0, (g_TalentStrength < 1.0) ? RoundToCeil(d_Damage * g_TalentStrength) : RoundToCeil(g_TalentStrength), false);
		}
...
stock AddCommonInfectedDamage(client, entity, playerDamage = 0, bool IsStatusDamage = false, damagetype = -1, ammotype = -1, hitgroup = -1) {
	...
	if (playerDamage > 0) {
		...
		commonHealthRemaining = RoundToCeil(commonHealthRemaining * (1.0 - CheckTeammateDamages(entity, client)));
...
stock AddWitchDamage(client, entity, playerDamageToWitch, bool IsStatusDamage = false, damagevariant = -1, ammotype = -1, hitgroup = -1) {
	...
	if (playerDamageToWitch >= 0) {
		...
		healthTotal = RoundToCeil(healthTotal * (1.0 - CheckTeammateDamages(entity, client)));
		//new TrueHealthRemaining = RoundToCeil((1.0 - CheckTeammateDamages(entity, client)) * healthTotal);	// in case other players have damaged the mob - we can't just assume the remaining health without comparing to other players.
		if (damagevariant != 2) {
...
bool ForceClearSpecialCommon(entity) {
	int pos		= FindListPositionByEntity(entity, CommonList);
...
stock void CreateItemDrop(owner, client) {
	...
	//DispatchKeyValue(entity, "glowcolor", "255 255 255 255");
