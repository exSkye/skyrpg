 ...
 */
stock GetConfigValue(String:TheString[], TheSize, String:KeyName[], String:sDefaultVal[] = "-1") {
...
stock GetConfigValueInt(String:KeyName[], defaultValue = -1) {
...
stock GetShotgunPelletCount(client) {
...
stock GetBaseWeaponDamage(client, target, Float:impactX = 0.0, Float:impactY = 0.0, Float:impactZ = 0.0, damagetype, bool:bGetBaseDamage = false, bool:IsDataSheet = false) {
	...
	new dontActivateTalentCooldown = (IsDataSheet) ? 1 : 0;
	...
	for (new i = 0; i < size; i++) {
		...
		if (IsDataSheet) baseWeaponTemp += RoundToCeil(GetAbilityStrengthByTrigger(client, target, "D", _, WeaponDamage, _, _, "d", 1, true, _, _, _, damagetype, dontActivateTalentCooldown));
		...
		if (fSurvivorBufferBonus > 0.0 && IsSpecialCommonInRange(client, 'b')) baseWeaponTemp += RoundToCeil(WeaponDamage * fSurvivorBufferBonus);
 		...
 		if (IsClientInRangeSpecialAmmo(client, "d") == -2.0) baseWeaponTemp += RoundToCeil(WeaponDamage * IsClientInRangeSpecialAmmo(client, "d", false));
 		if (IsClientInRangeSpecialAmmo(client, "E") == -2.0) baseWeaponTemp += RoundToCeil(WeaponDamage * IsClientInRangeSpecialAmmo(client, "E", false));
 		if (IsLegitimateClientAlive(target) && IsClientInRangeSpecialAmmo(target, "E") == -2.0) baseWeaponTemp += RoundToCeil(WeaponDamage * IsClientInRangeSpecialAmmo(target, "E", false));
		//}
		//else
		...
		if (!IsMelee) {
			WeaponRange += GetAbilityStrengthByTrigger(client, target, "gunRNG", _, RoundToCeil(WeaponRange), _, _, "d", 1, true, _, _, _, damagetype, dontActivateTalentCooldown);
			baseWeaponTemp += RoundToCeil(GetAbilityStrengthByTrigger(client, target, "gunDMG", _, WeaponDamage, _, _, "d", 1, true, _, _, _, damagetype, dontActivateTalentCooldown));

			if (bIsInCombat[client]) {
				WeaponRange += GetAbilityStrengthByTrigger(client, target, "ICwRNG", _, RoundToCeil(WeaponRange), _, _, "d", 1, true, _, _, _, damagetype, dontActivateTalentCooldown);
				baseWeaponTemp += RoundToCeil(GetAbilityStrengthByTrigger(client, target, "ICwDMG", _, WeaponDamage, _, _, "d", 1, true, _, _, _, damagetype, dontActivateTalentCooldown));
			}
			else {
				WeaponRange += GetAbilityStrengthByTrigger(client, target, "OCwRNG", _, RoundToCeil(WeaponRange), _, _, "d", 1, true, _, _, _, damagetype, dontActivateTalentCooldown);
				baseWeaponTemp += RoundToCeil(GetAbilityStrengthByTrigger(client, target, "OCwDMG", _, WeaponDamage, _, _, "d", 1, true, _, _, _, damagetype, dontActivateTalentCooldown));
			}
		}
		else {
			baseWeaponTemp += RoundToCeil(GetAbilityStrengthByTrigger(client, target, "mDMG", _, WeaponDamage, _, _, "d", 1, true, _, _, _, damagetype, dontActivateTalentCooldown));
		}
		...
		TheAbilityMultiplier = GetAbilityStrengthByTrigger(client, target, "lessDamageMoreHeals", _, WeaponDamage, _, _, "d", 1, true, _, _, _, damagetype, dontActivateTalentCooldown);
...
stock DoesClientHaveTheHighGround(Float:cpos[3], target) {
	new Float:tpos[3];
	GetEntPropVector(target, Prop_Send, "m_vecOrigin", tpos);
...
stock Float:GetTargetRange(client, target) {
	new Float:cpos[3];
	new Float:tpos[3];
...
stock CheckTankSubroutine(tank, survivor = 0, damage = 0, bool:TankIsVictim = false) {
	...
	new tankFlags = GetEntityFlags(tank);
	...
	new bool:IsLegitimateClientSurvivor = IsLegitimateClient(survivor);
	new survivorTeam = -1;
...
public Action:OnTakeDamage(victim, &attacker, &inflictor, &Float:damage_ignore, &damagetype) {
	...
	new Float:AltAbilityMultiplier = 0.0;
	new cTank = -1;
	...
	*/
	new bool:IsLegitimateClientVictim = IsLegitimateClient(victim);
	new victimTeam = -1;
	new bool:IsFakeClientVictim;
		...
		if (victimTeam == TEAM_SURVIVOR) {
			...
			if (attackerType <= 3) {
				...
				if (attackerType == 3) {
					...
					AltAbilityMultiplier = GetAbilityMultiplier(victim, "expo");
					...
					if (TheAbilityMultiplier >= 1.0) {	// Damage taken reduced to 0.
						...
						i_WitchDamage += RoundToCeil(i_WitchDamage * (TheAbilityMultiplier * -1.0));
					}
					new Float:fDirectorPointAward = (fWitchDirectorPoints * i_WitchDamage);
					if (!IsSurvivalMode && RPGRoundTime() >= iEnrageTime) fDirectorPointAward *= fEnrageDirectorPoints;
					if (fDirectorPointAward > 0.0) {
						// decl String:WitchName[64];
						// Format(WitchName, sizeof(WitchName), "%t", "witch");
						// PrintToChatAll("%t", "director points earned", orange, green, fDirectorPointAward, orange, WitchName);
						Points_Director += fDirectorPointAward;
					}
				...
				if (!StrEqual(effectToCreate, "-1")) {
					...
					if (iBurnCounter < iDebuffLimit && fOnFireDebuff[victim] <= 0.0) {
						...
						if (fOnFireDebuff[victim] == -1.0) {
							...
							if (iRPGMode >= 1) {
								new Float:fAcidDamage = (attackerType == 1) ? fAcidDamageSupersPlayerLevel : fAcidDamagePlayerLevel;
								if (iBotLevelType == 1) {
...
stock IfCommonInfectedIsAttackerDoStuff(attacker, victim, damagetype, survivorCount) {
	...
	if (!(damagetype & DMG_DIRECT)) {
		...
		new Float:AltAbilityMultiplier = GetAbilityMultiplier(victim, "expo");
		...
		if (TheAbilityMultiplier >= 1.0) return -1;
			...
			CommonsDamage += RoundToCeil(CommonsDamage * (TheAbilityMultiplier * -1.0));
		}
		...
		new Float:fCommonDirectorAward = (fCommonDirectorPoints * CommonsDamage);
	...
	new bool:attackerIsSpecial = IsSpecialCommon(attacker);
...
stock IfInfectedIsAttackerDoStuff(attacker, victim) {
	...
	if (TheAbilityMultiplier >= 1.0) return -1;
		...
		totalIncomingDamage += RoundToCeil(totalIncomingDamage * (TheAbilityMultiplier * -1.0));
	}
	...
	new infectedZombieClass = FindZombieClass(attacker);
	...
	if (IsFakeClient(attacker)) {
		...
		//new Float:fDirectorPointAward;
		// if (IsSurvivalMode || RPGRoundTime() < iEnrageTime) fDirectorPointAward += (totalIncomingDamage * fPointsMultiplierInfected);
		// else fDirectorPointAward += ((totalIncomingDamage * fPointsMultiplierInfected) * fEnrageDirectorPoints);
		// if (fDirectorPointAward > 0.0 && infectedZombieClass != ZOMBIECLASS_SPITTER) {
		// 	Points_Director += fDirectorPointAward;
		// 	PrintToChatAll("%t", "director points earned", orange, green, fDirectorPointAward, orange);
		// }
	}
	...
	TheAbilityMultiplier = GetAbilityStrengthByTrigger(victim, attacker, "lessTankyMoreHeals", _, totalIncomingDamage, _, _, "o", _, true, _, _, _, _, 1);
...
stock IfSurvivorIsAttackerDoStuff(attacker, victim, baseWeaponDamage, damagetype, victimType) {
	new infectedResult = (victimType <= 2) ? TryToDamageNonPlayerInfected(attacker, victim, baseWeaponDamage, damagetype) : 0;
	if (victimType <= 4) {
	...
	if (victimType <= 2) {
		...
		if (infectedResult > 1) {
			...
			if (iDeleteCommonsFromExistenceOnDeath == 1 || iDeleteCommonsFromExistenceOnDeath > 1 && GetArraySize(CommonAffixes) >= iDeleteCommonsFromExistenceOnDeath) AcceptEntityInput(victim, "Kill");
	...
	if (victimType == 3) {
		...
		if (IsSpecialCommonInRange(attacker, 'd') && (damagetype & DMG_BULLET)) {
			SetClientTotalHealth(attacker, baseWeaponDamage);
		}
	}
	
	decl String:weapon[64];
	FindPlayerWeapon(attacker, weapon, sizeof(weapon));
	if (bIsMeleeCooldown[attacker] && (StrContains(weapon, "melee", false) != -1 || StrContains(weapon, "chainsaw", false) != -1)) return -1;
	return 0;
}
...
stock TryToDamageNonPlayerInfected(attacker, victim, baseWeaponDamage, damagetype) {
	...
	new bool:isDifferentVictim = (LastAttackedUser[attacker] == victim) ? false : true;
	decl String:weapon[64];
	decl String:TheCommonValue[10];
	new bool:bIsMeleeAttack = IsMeleeAttacker(attacker);
	FindPlayerWeapon(attacker, weapon, sizeof(weapon));
	new bool:isVictimSpecial = IsSpecialCommon(victim);
	if (!bIsMeleeAttack || isDifferentVictim || !bIsMeleeCooldown[attacker]) {
		new bool:allowshotgun = true;// AllowShotgunToTriggerNodes(attacker);
		if (!bIsMeleeCooldown[attacker] && bIsMeleeAttack) {
			bIsMeleeCooldown[attacker] = true;				
			CreateTimer(0.25, Timer_IsMeleeCooldown, attacker, TIMER_FLAG_NO_MAPCHANGE);
		}
		...
		if (IsWitch(victim)) {
			...
			if (AddSpecialCommonDamage(attacker, victim, baseWeaponDamage) > 1) return baseWeaponDamage;
		}
		else if (IsCommonInfected(victim)) {
			if (AddCommonInfectedDamage(attacker, victim, baseWeaponDamage) > 1) return baseWeaponDamage;
		}
...
stock AddSpecialInfectedDamage(client, target, TotalDamage, bool:IsTankingInstead = false, damagevariant = -1) {
	...
	new i_InfectedCurrent = 0;
	if (!IsTankingInstead) i_InfectedCurrent = GetArrayCell(Handle:InfectedHealth[client], isEntityPos, 2);
	else i_InfectedCurrent = GetArrayCell(Handle:InfectedHealth[client], isEntityPos, 3);
...
stock AddSpecialCommonDamage(client, entity, playerDamage, bool:IsStatusDamage = false, damagevariant = -1) {
	...
	if (my_pos < 0) {
		new CommonHealth = GetCommonValueIntAtPos(entity, SUPER_COMMON_BASE_HEALTH);
	...
	if (playerDamage >= 0) {
		...
		if (playerDamage > 0) {
			...
			if (CheckTeammateDamagesEx(client, entity, playerDamage) > 0) return playerDamage;
		}
	}
	// else {
...
stock Float:GetPassiveInfo(client, target, Handle:Keys, Handle:Values, String:TalentName[], bool:bIsCreateCooldown = false) {
	new Float:f_EachPoint	= GetTalentInfo(client, Values, 1, _, TalentName, target);
	new Float:f_Cooldown	= GetTalentInfo(client, Values, 3, _, TalentName, target);
...
stock TalentRequirementsMet(client, Handle:Keys, Handle:Values, String:sTalentList[] = "none", TheSize = 0, requiredTalentsToUnlock = 0) {
	new pos = TALENT_FIRST_RANDOM_KEY_POSITION;
	decl String:TalentName[64];
	decl String:text[64];
...
stock bool:IsStatusEffectFound(client, Handle:Keys, Handle:Values) {
	decl String:statusEffectToSearchFor[64];
	new pos = TALENT_FIRST_RANDOM_KEY_POSITION;
	while (pos >= 0) {
...
stock Float:GetAbilityStrengthByTrigger(activator, target = 0, String:AbilityT[], zombieclass = 0, damagevalue = 0,
	...
	if (activatorIsSurvivor) GetClientWeapon(activator, PlayerWeapon, sizeof(PlayerWeapon));
	...
	new bool:ledgeState = (incapState) ? IsLedged(activator) : false;
	new Float:fPercentageHealthRemaining = ((GetClientHealth(activator) * 1.0) / (GetMaximumHealth(activator) * 1.0));
	new Float:fPercentageHealthRequired = 0.0;
	new Float:fPercentageHealthRequiredMax = 0.0;
	new Float:fPercentageHealthMissing = 1.0 - fPercentageHealthRemaining;
	new iSurvivorsInRange = 0;
	new iCoherencyMax = 0;
	new Float:fPercentageHealthTargetRemaining = GetClientHealthPercentage(activator, target, true);
	new Float:fPercentageHealthTargetMissing = 1.0 - fPercentageHealthTargetRemaining;
	new Float:fTargetRangeRequired = 0.0;
	new Float:fTargetRange = GetTargetRange(activator, target);
	new bool:bTargetMustBeWithinRange = false;
	new bool:bTalentRequiresLimbshot = false;
	new bool:bTalentRequiresHeadshot = false;
	new bool:bTargetIsLastTarget = (target == lastTarget[activator]) ? true : false;
	new iLastTargetResult = -1;
	new bool:activatorIsOnFire = (GetClientStatusEffect(activator, "burn") > 0) ? true : false;
	new bool:activatorIsSufferingAcidBurn = (GetClientStatusEffect(activator, "acid") > 0) ? true : false;
	new bool:activatorIsExploding = (ISEXPLODE[activator] != INVALID_HANDLE) ? true : false;
	new bool:activatorIsSlowed = ((ISSLOW[activator] != INVALID_HANDLE || playerInSlowAmmo[activator]) && fSlowSpeed[activator] < 1.0) ? true : false;
	...
	for (new i = 0; i < ASize; i++) {
		...
		if (countAllTalentsRegardlessOfState == 0 && IsAbilityCooldown(activator, TalentName)) continue;
		if (!RollChanceIsSuccessful(activator, GetKeyValueFloatAtPos(TriggerValues[activator], TALENT_ROLL_CHANCE))) continue;
		if (!LastHitWasHeadshot[activator] && GetKeyValueIntAtPos(TriggerValues[activator], LAST_KILL_MUST_BE_HEADSHOT) == 1) continue;
		if (!StrEqual(TargetClass, LastTargetClass[activator]) && GetKeyValueIntAtPos(TriggerValues[activator], TARGET_AND_LAST_TARGET_CLASS_MATCH) == 1) continue;
		...
		//if (ResultType >= 1) {
		// target ability effects can be called on resulttype 0
		...
		if (activatorIsSurvivor) {
			GetArrayString(TriggerValues[activator], WEAPONS_PERMITTED, WeaponsPermitted, sizeof(WeaponsPermitted));
		...
		GetArrayString(TriggerValues[activator], ACTIVATOR_TEAM_REQ, TheString, sizeof(TheString));
		if (!StrEqual(TheString, "-1") && StrContains(TheString, activatorteam) == -1) continue;
		GetArrayString(TriggerValues[activator], ACTIVATOR_CLASS_REQ, TheString, sizeof(TheString));
		...
		if (countAllTalentsRegardlessOfState == 0 && f_Cooldown > 0.0) CreateCooldown(activator, GetTalentPosition(activator, TalentName), f_Cooldown);
...
stock bool:ComparePlayerState(client, String:CombatState[], bool:incapState, bool:ledgeState, infectedAttacker) {
...
stock GetTalentKeyValue(client, String:TalentName[], pos, String:storage[], storageLocSize) {
	new size = GetArraySize(a_Menu_Talents);
...
stock GetTalentStrengthByKeyValue(client, pos, String:searchValue[], bool:getFirstTalentResult = true) {
	new size = GetArraySize(a_Menu_Talents);
	decl String:result[64];
	new count = 0;
	new total = 0;
...
stock FormatKeyValue(String:TheValue[], TheSize, Handle:Keys, Handle:Values, String:SearchKey[], String:DefaultValue[] = "none", bool:bDebug = false, pos = 0, bool:incrementPos = true) {
...
stock CreateFireEx(client) {
	if (IsLegitimateClient(client) || IsCommonInfected(client)) {
		decl Float:pos[3];
...
stock SetMaximumHealth(client) {
	...
	if (DefaultHealth[client] > 5000) DefaultHealth[client] = 5000;
...
stock Float:GetInfectedHealthPercentage(client) {
	new Float:healthRemaining = GetEntProp(client, Prop_Data, "m_iHealth") * 1.0;
	new Float:healthMax = GetEntProp(client, Prop_Data, "m_iMaxHealth") * 1.0;
...
stock ActivateAbilityEx(activator, target, d_Damage, String:Effects[], Float:g_TalentStrength, Float:g_TalentTime, victim = 0,
						...
						Float:secondaryAoERange = 0.0, hitgroup = -1, String:secondaryTrigger[] = "-1", String:AbilityTriggerIgnore[] = "none", damagetype = -1) {
	...
	*/
	...
	if (g_TalentStrength > 0.0) {
		...
		new iDamage = (isRaw == 1 || d_Damage == 0) ? RoundToCeil(g_TalentStrength) : RoundToCeil(d_Damage * g_TalentStrength);
		...
		// 	new bool:IsCommonTarget = false;
		// 	new bool:IsSpecialCommonTarget = false;
		// 	new bool:IsWitchTarget = false;
		...
		// 		AddCommonInfectedDamage(activator, target, iDamage);
		// 	}
		// 	else if (IsLegitimateClientAlive(target)) {
		// 		if (activatorTeam == TEAM_SURVIVOR && GetClientTeam(target) == activatorTeam && activator != target) {
		// 			new infAttacker = L4D2_GetInfectedAttacker(target);
		// 			if (infAttacker == -1) SameTeam_OnTakeDamage(activator, target, iDamage - oDamage, true, damagetype);
		// 			else target = infAttacker;
		// 		}
		...
		// There are 3 different result effects for giving ammo back, depending on the request.
			...
			//if (FindZombieClass(target) )
			if (iDamage > 0) ReflectDamage(activator, target, iDamage);
		}
...
stock GetEntitiesInRange(client, victim) {
	...
	new Float:AfxRange		= GetCommonValueFloatAtPos(client, SUPER_COMMON_RANGE_PLAYER_LEVEL) * PlayerLevel[victim];
...
stock bool:IsSpecialCommonInRange(client, Effect = -1, vEntity = -1, bool:IsAuraEffect = true, infectedTarget = 0) {	// false for death effect
	...
	if (vEntity != -1) {
		...
		new Float:AfxRange = GetCommonValueFloatAtPos(vEntity, SUPER_COMMON_RANGE_PLAYER_LEVEL);
		//new Float:AfxStrengthLevel = StringToFloat(GetCommonValue(vEntity, "level strength?"));
		new Float:AfxRangeMax = GetCommonValueFloatAtPos(vEntity, SUPER_COMMON_RANGE_MAX);
...
stock GetCommonValueAtPos(String:TheString[], TheSize, entity, valPos, pos = 0, bool:incrementNonZeroPos = true) {
	decl String:AffixName[2][64];
	new ent = -1;
...
stock Float:GetCommonValueFloatAtPos(entity, pos, String:Section_Name[] = "none") {	// can override the section
	decl String:AffixName[2][64];
	new ent = -1;
...
stock GetCommonValue(String:TheString[], TheSize, entity, String:Key[], pos = 0, bool:incrementNonZeroPos = true) {
	decl String:AffixName[2][64];
	new ent = -1;
	if (pos > 0 && incrementNonZeroPos) pos++;
	new size = GetArraySize(CommonAffixes);
...
stock DrawCommonAffixes(entity, String:sForceAuraEffect[] = "none") {
	...
	new AfxLevelReq = GetCommonValueIntAtPos(entity, SUPER_COMMON_LEVEL_REQ);
	...
	new t_Strength			= 0;
	new AfxStrength			= GetCommonValueIntAtPos(entity, SUPER_COMMON_AURA_STRENGTH);
	new AfxMultiplication	= GetCommonValueIntAtPos(entity, SUPER_COMMON_ENEMY_MULTIPLICATION);
	new Float:AfxStrengthLevel = GetCommonValueFloatAtPos(entity, SUPER_COMMON_LEVEL_STRENGTH);
...
public Action:Timer_SpecialAmmoData(Handle:timer, any:client) {
	...
	//new ent = -1;
	new drawtarget = -1;
	decl String:DataAmmoEffect[10];
	new Float:AmmoStrength = 0.0;
	new auraMenuPosition = 0;
	new bool:IsPlayerSameTeam;
	new dataAmmoType = 0;
	new bulletStrength = 0;
	new Float:fVisualDelay = 0.0;
	new numOfStatusEffects = GetClientStatusEffect(client);
...
public Action:Timer_StartPlayerTimers(Handle:timer) {
	...
	for (new i = 1; i <= MaxClients; i++) {
		...
		if (!IsPlayerAlive(i) || GetClientTeam(i) != TEAM_SURVIVOR || bTimersRunning[i] || !b_IsLoaded[i] || b_IsLoading[i]) continue;
...
public Action:Timer_AmmoActiveTimer(Handle:timer, any:client) {
	...
	new Float:talentTimeRemaining = 0.0;
	new triggerRequirementsAreMet = 0;
...
public Action:Timer_EffectOverTime(Handle:timer, any:client) {
	...
	decl String:periodicActivation[64];
	//static String:result[7][64];
	new Float:activeTime = 0.0;
	...
	decl String:secondaryTrigger[64];
	new target = 0;
	new Float:talentStrength = 0.0;
	decl String:AoERange[10];
	decl String:AoERange2[10];
	decl String:TalentName[64];
	decl String:primaryEffects[64];
...
stock bool:AllowEffectOverTimeToContinue(client, Handle:Values, String:TalentName[], damage, String:effects[] = "-1", target = 0, Float:fTalentStrength = 0.0) {
...
stock FormatEffectOverTimeBuffs(client, String:eBar[], eBarSize, pos = -1) {
	...
	for (new i = startpos; i < size; i++) {
		...
		if (CheckEffectOverTimeCooldown(client, TalentName, EFFECTOVERTIME_GETACTIVETIME) > 0.0) {
			GetArrayString(FormatEffectOverTimeValues[client], HUD_TEXT_BUFF_EFFECT_OVER_TIME, TalentName, sizeof(TalentName));
...
stock Float:EffectOverTimeActiveStatus(client, String:TalentName[], bool:activateTalent = false, bool:checkIfCooldownIsActive = false, damage = 0, String:effects[] = "-1", target = 0, Float:fTalentStrength = 0.0) {
	...
	if (activateTalent) {
		...
		new Float:effectOverTimeCooldown	= GetTalentInfo(client, CheckEffectOverTimeValues[client], 3, _, TalentName);
		new Float:effectActiveTime			= GetTalentInfo(client, CheckEffectOverTimeValues[client], 2, _, TalentName); 
...
stock bool:IsAmmoEffectActive(client, String:TalentName[]) {
	new Float:fCooldown = GetAmmoCooldownTime(client, TalentName);
	if (fCooldown == -1.0) return false;
	new Float:fAmmoCooldownTime = fCooldown;
	fCooldown = GetSpecialAmmoStrength(client, TalentName);

	new Float:fAmmoCooldown = fCooldown + GetSpecialAmmoStrength(client, TalentName, 1);
...
stock Float:GetAbilityValue(client, String:TalentName[], valuePos) {
...
bool:AbilityIsInactiveAndOnCooldown(client, String:TalentName[], Float:fCooldownRemaining) {
	...
	new Float:AmmoCooldownTime = GetAbilityValue(client, TalentName, ABILITY_ACTIVE_TIME);
...
stock Float:GetAbilityMultiplier(client, String:abilityT[], override = 0, String:TalentName_t[] = "none") { // we need the option to force certain results in the menus (1) active (2) passive
	...
	*/
	...
	for (new i = 0; i < size; i++) {
		...
		GetArrayString(Values, WEAPONS_PERMITTED, allowedWeapons, sizeof(allowedWeapons));
...
stock EntityStatusEffectDamage(client, damage) {
	...
	*/
	new pos = -1;
	new bool:ClientIsWitch = IsWitch(client);
	if (ClientIsWitch) pos = FindListPositionByEntity(client, Handle:WitchList);
	if (pos < 0) return;
	for (new i = 1; i <= MaxClients; i++) {
		if (!IsLegitimateClientAlive(i) || GetClientTeam(i) != TEAM_SURVIVOR) continue;
...
stock GetDifficultyRating(client, bool:JustBaseRating = false) {
	...
	new RatingPerLevelMultiplier = (IsSurvivorBot(client)) ? RatingPerLevelSurvivorBots : RatingPerLevel;

	if (iTankRush == 1) RatingHandicap[client] = RatingPerLevelMultiplier;
	new iRatingPerLevel = RatingPerLevelMultiplier;
	...
	if (iRatingPerLevel < RatingPerLevelMultiplier) iRatingPerLevel = RatingPerLevelMultiplier;
	if (!JustBaseRating) return iRatingPerLevel + Rating[client];
	return iRatingPerLevel;
}

stock AddCommonInfectedDamageNotStored(client, entity, playerDamage, bool:IsStatusDamage = false) {
	if (playerDamage < 1) return 1;
	new bool:islegitclient = (IsLegitimateClient(client) && GetClientTeam(client) == TEAM_SURVIVOR) ? true : false;

	new commonHP = GetEntProp(entity, Prop_Data, "m_iHealth");
	if (playerDamage < commonHP) {
		SetEntProp(entity, Prop_Data, "m_iHealth", commonHP - playerDamage);
		if (islegitclient) GetAbilityStrengthByTrigger(client, entity, "hurtcommon", _, playerDamage);
		return 1;
	}
	...
	if (IsStatusDamage) IgniteEntity(entity, 1.0);
	if (commonHP < 2) return 2;
	return commonHP;
}
...
stock AddCommonInfectedDamage(client, entity, playerDamage, bool:IsStatusDamage = false, damagevariant = -1) {
	...
	if (iDontStoreInfectedInArray == 1) {
		return AddCommonInfectedDamageNotStored(client, entity, playerDamage, IsStatusDamage);
	}
	...
	if (playerDamage > 0) {
		...
		if (playerDamage >= commonHealthRemaining) {
			...
			SetArrayCell(CommonInfectedHealth, pos, commonHealthRemaining - playerDamage);
...
stock bool:IsCommonInfectedDead(entity) {
	...
	if (iDontStoreInfectedInArray == 1) return true;

	if (FindListPositionByEntity(entity, Handle:CommonInfected) < 0) {
		SetInfectedHealth(entity, 1);
		return true;
	}
	return false;
}
...
stock GetCommonBaseHealth() {
	return (iCommonBaseHealth + RoundToCeil(iCommonBaseHealth * (GetTeamRatingAverage() * fCommonLevelHealthMult)));
}

stock OnCommonInfectedCreated(entity, bool:bIsDestroyed = false, finalkillclient=0, bool:IsIgnite = false) {
	if (CommonInfected == INVALID_HANDLE || IsSpecialCommon(entity) || iDontStoreInfectedInArray == 1) return;
	if (!b_IsActiveRound) return;
	new pos = FindListPositionByEntity(entity, Handle:CommonInfected);
	//new pos = FindEntityInArrayBinarySearch(CommonInfected, entity);
	//new ASize = GetArraySize(Handle:CommonInfected);
	if (!bIsDestroyed && pos < 0) {
		//InsertIntoArrayAscending(CommonInfected, entity);
		//InsertIntoArrayAscending(CommonInfectedHealth, iCommonBaseHealth + RoundToCeil(iCommonBaseHealth * (GetTeamRatingAverage() * fCommonLevelHealthMult)));
		PushArrayCell(Handle:CommonInfected, entity);
		PushArrayCell(Handle:CommonInfectedHealth, GetCommonBaseHealth());
	}
		...
		RemoveFromArray(Handle:CommonInfectedHealth, pos);
		//OnCommonCreated(entity, true, true);
		//RemoveCommonAffixes(entity);
		SDKUnhook(entity, SDKHook_OnTakeDamage, OnTakeDamage);
...
public OnEntityDestroyed(entity) {
	...
	if (iDontStoreInfectedInArray == 0) {
		OnCommonInfectedCreated(entity, true);
		new ent = FindListPositionByEntity(entity, CommonInfected);
		if (ent >= 0) RemoveFromArray(CommonInfected, ent);
	}
	SDKUnhook(entity, SDKHook_OnTakeDamage, OnTakeDamage);
}
...
stock OnEntityCreatedEx(entity, const String:classname[], bool:creationOverride = false) {
	...
	if (creationOverride || IsCommonInfected(entity)) {
		...
		if (CreateCommonAffix(entity) == 0) {
			...
			if (iDontStoreInfectedInArray == 0) OnCommonInfectedCreated(entity);
			else {
				// set infected health here.
				new commonHP = GetCommonBaseHealth();
				SetEntProp(entity, Prop_Data, "m_iHealth", commonHP);
				SetEntProp(entity, Prop_Data, "m_iMaxHealth", commonHP); 
				//PushArrayCell(CommonInfected, entity);
			}
		}
		SDKHook(entity, SDKHook_OnTakeDamage, OnTakeDamage);
	}
...
stock GetStatusEffects(client, EffectType = 0, String:theStringToStoreItIn[], theSizeOfTheString) {
	...
	if (EffectType == 0) {
		...
		new Float:isSlowSpeed = IsClientInRangeSpecialAmmo(client, "s");
		if (ISSLOW[client] != INVALID_HANDLE || isSlowSpeed == -2.0) {
			if (isSlowSpeed == -2.0) playerInSlowAmmo[client] = true;
...
stock GetPlayerStamina(client) {
	...
	*/
	...
	new StaminaMax = iSurvivorStaminaMax + RoundToCeil(PlayerLevel[client] * fStaminaPerPlayerLevel) + RoundToCeil(SkyLevel[client] * fStaminaPerSkyLevel);
	new StaminaBonus = RoundToCeil(GetAbilityStrengthByTrigger(client, client, "getstam", _, 0, _, _, "stamina", _, _, 2));
	...
	if (iRPGMode >= 1) {
		...
		if (bJetpack[client]) {
			...
			if (enemytype != 2 || GetClientTeam(enemycombatant) != TEAM_SURVIVOR) {
				...
				Format(text, sizeof(text), "%s\n%sHP\n%s", EnemyName, targetHealthText, clientStatusEffectDisplay[client]);
			}
...
stock Float:GetClientHealthPercentageSurvivor(client, bool:GetHealthRemaining = false) {
	new Float:fHealthRemaining = (GetClientHealth(client) * 1.0) / (GetMaximumHealth(client) * 1.0);
...
stock Float:GetClientHealthPercentage(client, target, bool:GetHealthRemaining = false) {
	new pos = -1;
	if (IsLegitimateClient(target)) {
		if (GetClientTeam(target) == TEAM_INFECTED) pos = FindListPositionByEntity(target, Handle:InfectedHealth[client]);
	...
	else if (IsSpecialCommon(target)) pos = FindListPositionByEntity(target, Handle:SpecialCommon[client]);
	else if (IsCommonInfected(target)) {
		if (iDontStoreInfectedInArray == 0) pos = FindListPositionByEntity(target, Handle:CommonInfected);
		else return GetInfectedHealthPercentage(target);
	}
	if (pos < 0) return -1.0;
	new Float:fHealthRemaining = CheckTeammateDamages(target, client);
...
stock TruRating(client) {
	...
	new TrueRating = (IsSurvivorBot(client)) ? RatingPerLevelSurvivorBots : RatingPerLevel;
...
stock IncapacitateOrKill(client, attacker = 0, healthvalue = 0, bool:bIsFalling = false, bool:bIsLifelinkPenalty = false, bool:ForceKill = false) {
	new bool:isPlayerASurvivorBot = IsSurvivorBot(client);
	...
	if ((ForceKill || IsLegitimateClientAlive(client)) && (GetClientTeam(client) != TEAM_INFECTED || isPlayerASurvivorBot)) {
		...
		if (ForceKill || isClientIncapacitated || bIsFalling && !IsLedged(client) || bHasWeakness[client] || GetIncapCount(client) >= iMaxIncap) {
			...
			new infectedAttacker = L4D2_GetInfectedAttacker(client);
...
stock bool:CheckIfHeadshot(attacker, victim, Handle:event, damage) {
	if (GetClientTeam(attacker) == TEAM_SURVIVOR) {
		...
		if (hitgroup == 1) {
			...
			if (IsCommonInfected(victim) && !IsSpecialCommon(victim)) {
				AddCommonInfectedDamage(attacker, victim, GetCommonBaseHealth());	// if someone shoots a common infected in the head, we want to auto-kill it.
			}
