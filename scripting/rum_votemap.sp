...
#define PLUGIN_NAME			"[ReadyUp! Module] Vote Map"
...
public OnPluginStart() {
	CreateConVar("rum_votemap", PLUGIN_VERSION, "version header", FCVAR_NOTIFY);
...
public Action CMD_VoteMap(int client, args) {
...
stock RestrictMap(char[] Mapname_a) {
...
stock bool IsMapAllowed(char[] Mapname_a) {
...
stock StartVotemap() {
	...
	if (GetArraySize(a_MapListRestricted) != i_RestrictCount) {
		...
		for (int i = 0; i < GetArraySize(a_MapListRestricted); i++) {
...
stock void FindMapDescription(char[] Name, char[] storage, int storageSize) {
	int a_Size = GetArraySize(a_MapListIdentifier);
	for (int i = 0; i < a_Size; i++) {
		char text[64];
		GetArrayString(a_MapListIdentifier, i, text, sizeof(text));
		if (StrEqual(Name, text)) {
			GetArrayString(a_MapListDescription, i, storage, sizeof(storageSize));
			break;
		}
	}
	strcopy(storage, sizeof(storageSize), Name);
}
...
public ReadyUp_TrueDisconnect(int client) {
...
public ReadyUp_CoopMapFailed(int gamemode) {
	if (gamemode == 1) {
		i_Rounds++;
...
stock bool IsChangeMap() {
	...
	LowerString(mapname, mapname, sizeof(mapname));
	char a_mapname[128];
	int a_Size = GetArraySize(a_MapList2);
	for (int i = 0; i < a_Size; i++) {
		GetArrayString(a_MapList2, i, a_mapname, sizeof(a_mapname));
		LowerString(a_mapname, a_mapname, sizeof(a_mapname));
		if (StrEqual(mapname, a_mapname)) return true;
	}
	return false;
}

stock void LowerString(char[] s, char[] text, int textSize) {
	for (int i = 0; i <= strlen(s); i++) {
		if (!IsCharLower(s[i])) text[i] = CharToLower(s[i]);
		else text[i] = s[i];
	}
}

stock int GetHumanPlayers(int client = -1) {
	int count = 0;
	for (int i = 1; i <= MaxClients; i++) {
