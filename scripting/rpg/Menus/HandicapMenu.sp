stock HandicapMenu(client) {
	...
	for (int i = 0; i < size; i++) {
		...
		char damage[10];
		AddCommasToString(RoundToNearest(handicapDamage * 100.0), damage, 10);
		char health[10];
		AddCommasToString(RoundToNearest(handicapHealth * 100.0), health, 10);
		char lootfind[10];
		AddCommasToString(RoundToNearest((lootFindBonus * fAugmentRatingMultiplier) * 100.0), lootfind, 10);
		char scorebonus[10];
		AddCommasToString(RoundToNearest(scoreMult * 100.0), scorebonus, 10);
