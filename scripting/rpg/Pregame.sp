...
public ReadyUp_CheckpointDoorStartOpened() {
	if (!b_IsCheckpointDoorStartOpened) {
		...
		int iSurvivorLimit = GetConVarInt(FindConVar("survivor_limit"));
		int numberOfHumansMissing = iSurvivorLimit - TotalHumanSurvivors() + 1;
		float fSmallerPartyTimeToAdd		= ((numberOfHumansMissing > iMaximumEnrageLeniency) ? iMaximumEnrageLeniency : numberOfHumansMissing) * fEnragePeriodLeniency;
		if (fSmallerPartyTimeToAdd > 0.0) {
			int iEnrageTimePityBonus = RoundToCeil(iEnrageTime * fSmallerPartyTimeToAdd);
			iEnrageTime += iEnrageTimePityBonus;
			TimeUntilEnrage(text, sizeof(text), iEnrageTimePityBonus);
			PrintToChatAll("%t", "enrage pity increase due to party size", green, text, orange);
		}
