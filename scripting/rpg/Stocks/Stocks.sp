...
stock TimeUntilEnrage(char[] TheText, int TheSize, int override = 0) {
	if (!IsEnrageActive()) {
		if (override == 0) override = iEnrageTime;
		int Seconds = (override * 60) - (GetTime() - RoundTime);
		int Minutes = 0;
		while (Seconds >= 60) {
			Seconds -= 60;
			Minutes++;
...
stock void ChangeInfectedClass(int client, int zombieclass) {
	...
	if (isLegitimate) {
		if (myCurrentTeam[client] == TEAM_INFECTED) {
			...
			if (clientIsFake) {
				if (zombieclass == 1) {
					SetClientInfo(client, "name", "Smoker");
					L4D2_SetEntityGlow(client, L4D2Glow_Constant, 500, 1, {0, 80, 0}, false);
				}
				else if (zombieclass == 2) {
					SetClientInfo(client, "name", "Boomer");
					L4D2_SetEntityGlow(client, L4D2Glow_Constant, 500, 1, {128, 0, 128}, false);
				}
				else if (zombieclass == 3) {
					SetClientInfo(client, "name", "Hunter");
					L4D2_SetEntityGlow(client, L4D2Glow_Constant, 500, 1, {80, 0, 0}, false);
				}
				else if (zombieclass == 4) {
					SetClientInfo(client, "name", "Spitter");
					L4D2_SetEntityGlow(client, L4D2Glow_Constant, 500, 1, {0, 80, 0}, false);
				}
				else if (zombieclass == 5) {
					SetClientInfo(client, "name", "Jockey");
					L4D2_SetEntityGlow(client, L4D2Glow_Constant, 500, 1, {80, 0, 0}, false);
				}
				else if (zombieclass == 6) {
					SetClientInfo(client, "name", "Charger");
					L4D2_SetEntityGlow(client, L4D2Glow_Constant, 500, 1, {80, 0, 0}, false);
				}
				else if (zombieclass == 8) {
					SetClientInfo(client, "name", "Tank");
					L4D2_SetEntityGlow(client, L4D2Glow_Constant, 500, 1, {80, 0, 0}, false);
				}
			}
