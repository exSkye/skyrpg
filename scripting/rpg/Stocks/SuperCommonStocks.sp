...
stock int CreateCommonAffix(int entity) {
	...
	for (int i = 0; i < size; i++) {
		...
		int isCommonGlowing = GetArrayCell(CCAValues, SUPER_COMMON_GLOW);
		if (isCommonGlowing == 1) {
			int commonGlowRange = GetArrayCell(CCAValues, SUPER_COMMON_GLOW_RANGE);
			int redGlow = GetArrayCell(CommonDrawGlows, i);
			int blueGlow = GetArrayCell(CommonDrawGlows, i, 1);
			int greenGlow = GetArrayCell(CommonDrawGlows, i, 2);
			L4D2_SetEntGlow(entity, L4D2Glow_Constant, commonGlowRange, 1, redGlow, blueGlow, greenGlow, false);
		}
...
stock void DrawCommonAffixes(entity, int superPos) {
...
// Draw color: -1 draw pos: -1
	...
	for (int i = 1; i <= MaxClients; i++) {
		...
		if (GetVectorDistance(clientPos, EntityPos) > fSuperCommonVisualEffectCullRange) continue;
		// drawing these effects is actually quite intensive so if the client is really far from the enemy
		// it now skips drawing those effects for that enemy. Scalable in config with "super common visuals cull range?" , default 2048.0 units.
