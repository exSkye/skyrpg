public Action OnPlayerRunCmd(client, &buttons) {
	...
	bool clientHasAdrenaline = (clientIsSurvivor) ? HasAdrenaline(client) : false;
		...
		if (clientIsSurvivor) {
			...
			if (freezerCheck[client] < TheTime && freezerTime[client] < TheTime) {
				...
				if (IsSpecialCommonInRange(client, 'r')) {
					...
					if (!clientHasAdrenaline) SetEntPropFloat(client, Prop_Send, "m_flLaggedMovementValue", 0.25);
					else SetEntPropFloat(client, Prop_Send, "m_flLaggedMovementValue", 0.75);
				}
				else {
					FreezerInRange[client] = false;
					if (!clientHasAdrenaline) SetEntPropFloat(client, Prop_Send, "m_flLaggedMovementValue", 1.0);
					else SetEntPropFloat(client, Prop_Send, "m_flLaggedMovementValue", 1.5);
				}
			}
						...
						if (theClientHasAnActiveProgressBar) {
							...
							if (!IsClientIncapacitated) {
								...
								if (theClientHasPainPills) {
									...
									float fAdrenHeal = GetTempHealth(healTarget) + (GetMaximumHealth(healTarget) * fAdrenHealAmount);
									HealPlayer(client, healTarget, fAdrenHeal, 'h', true);
				...
				*/
				...
				if (!bJetpack[client]) {
					if (SurvivorStaminaTime[client] < TheTime && SurvivorStamina[client] < PlayerMaxStamina) {
						if (!clientHasAdrenaline) SurvivorStaminaTime[client] = TheTime + fStamRegenTime;
