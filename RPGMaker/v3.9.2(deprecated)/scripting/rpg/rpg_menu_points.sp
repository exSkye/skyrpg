	...
	if (action == MenuAction_Select) {
		...
		if (FindCharInString(Command, ':') != -1) {
			...
			if (!StrEqual(MenuName, "director menu")) {
				...
				if (menuPos < GetArraySize(a_DirectorActions_Cooldown)) GetArrayString(a_DirectorActions_Cooldown, menuPos, IsCooldown, sizeof(IsCooldown));
				if (StringToInt(IsCooldown) > 0 && StringToInt(GetConfigValue("menu override director cooldown?")) == 0) PrintToChat(client, "%T", "Menu Option is On Cooldown", client, green, Name, white);
				else {

					if (StringToInt(GetConfigValue("points purchase type?")) == 0) {
					...
					else if (StringToInt(GetConfigValue("points purchase type?")) == 1) {
