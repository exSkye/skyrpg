	...
	if (IsLegitimateClientAlive(victim) && (GetClientTeam(victim) == TEAM_SURVIVOR || IsSurvivorBot(victim))) {
		...
		if (damagetype & DMG_FALL || !IsLegitimateClient(attacker) && !IsCommonInfected(attacker) && !IsWitch(attacker)) {//CheckFallDamage(victim);
			...
			if (damagetype & DMG_FALL) {
				...
				if (MyInfectedAttacker != -1) DMGFallDamage = RoundToCeil(DMGFallDamage * 0.5);
...
stock AwardExperience(client, type = 0, AMOUNT = 0, bool:TheRoundHasEnded=false) {
	...
	if (type == -1 && !b_IsLoading[client] && CurrentMapPosition != 0 && BonusContainer[client] > 0 && RoundExperienceMultiplier[client] > 0.0) {	//	This occurs when a player fully loads-in to a game, and is bonus container from previous round.
...
stock Float:GetClassMultiplier(client, Float:value, String:ClassEffect[], catType = 0, bool:AddingToValue = false) {

	if (!IsLegitimateClient(client) ||
		IsFakeClient(client) && GetClientTeam(client) == TEAM_INFECTED ||
	 !IsLegitimateClass(client) ||
...
stock Float:GetAbilityStrengthByTrigger(activator, target = 0, Ability, zombieclass = 0, damagevalue = 0, bool:IsOverdriveStacks = false, bool:IsCleanse = false, String:ResultEffects[] = "none", ResultType = 0) {	// activator, target, trigger ability, survivor effects, infected effects, common effects, zombieclass, damage
	...
	if (iRPGMode <= 0 ||
		!IsLegitimateClient(activator) ||
		(GetClientTeam(activator) == TEAM_SURVIVOR || IsSurvivorBot(activator)) && !IsLegitimateClass(activator)) return 0.0;
	if (GetClientTeam(activator) == TEAM_INFECTED) return 1.0;
	// ResultType:
	// 0 Activator
	// 1 Target
	...
	if (target < 1 || !IsLegitimateClient(target)) target = activator;
	...
	*/
	...
	new bool:iHasWeakness = false;
	new bool:iHasAdren = false;
	if (GetClientTeam(activator) == TEAM_SURVIVOR) {

		iHasWeakness = PlayerHasWeakness(activator);
		iHasAdren = HasAdrenaline(activator);
	}
	...
	new iActivatorTeam = GetClientTeam(activator);
	new iTargetTeam = GetClientTeam(target);
	new bool:TargetTeamIsSurvivor = false;
	if (target != activator && IsLegitimateClient(target)) {

		if (iTargetTeam == TEAM_SURVIVOR) {

			talenttarget = target;
			TargetTeamIsSurvivor = true;
		}
	}

	new bool:IsNotAnInfectedBot = true;
	if (IsFakeClient(activator) && iActivatorTeam == TEAM_INFECTED) IsNotAnInfectedBot = false;
	...
	for (new i = 0; i < ASize; i++) {
		...
		if (bIsCompounding || ResultType == 1) {
			...
			if (ResultType == 1) {
				...
				else if (IsLegitimateClient(target) && (iTargetTeam == TEAM_SURVIVOR || IsSurvivorBot(target)) && StrContains(ResultEffects, targeteffects, true) == -1) continue;
				else if (IsLegitimateClient(target) && iTargetTeam == TEAM_INFECTED && StrContains(ResultEffects, targeteffects, true) == -1) continue;
			}
		}
		...
		//else if (IsLegitimateClient(activator) && IsFakeClient(activator) && iActivatorTeam == TEAM_INFECTED) {
			...
			if (TargetTeamIsSurvivor) {
				...
				TheTalentStrength = TeamRatingBonus;
			}
		...
		if (iActivatorTeam == TEAM_SURVIVOR) {
			...
			if (!iHasAdren && GetKeyValueInt(TriggerKeys[activator], TriggerValues[activator], "require adrenaline effect?") == 1) continue;
		...
		if (target != activator) {
			...
			if (IsLegitimateClient(target)) {
				...
				if ((iTargetTeam == TEAM_SURVIVOR || IsSurvivorBot(target)) && StrContains(targeteffects, "0", false) != -1) continue;
				if (iTargetTeam == TEAM_INFECTED && StrContains(targeteffects, "0", false) != -1) continue;
				Format(targetteam, sizeof(targetteam), "%d", iTargetTeam);
			}
		}
		...
		else if (IsLegitimateClient(target)) Format(targetteam, sizeof(targetteam), "%d", iTargetTeam);
		...
		if (f_Cooldown > 0.00) {
			...
			if (IsLegitimateClient(activator)) {
				...
				if (!IsFakeClient(activator)) CreateCooldown(activator, GetTalentPosition(activator, TalentName), f_Cooldown);
			}
...
stock bool:QuickCommandAccess(client, args, bool:b_IsTeamOnly) {
	...
	if ((iRPGMode != 1 || !b_IsActiveRound) && iRPGMode >= 0) {
